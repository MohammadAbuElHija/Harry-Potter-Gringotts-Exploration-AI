...
import inputs
import inputs_100
from utils import pl_true, expr, find_unit_clause, dpll_satisfiable, remove_all, conjuncts, find_pure_symbol, to_cnf
...
class GringottsChecker(Checker):
    ...
    def check_controller(self):
        ...
        controller = ex2_worked.GringottsController(map_dimensions, self.harry_cur_loc, observations)
...
if __name__ == '__main__':
   ...
   excedded = 0
   illegal = 0
   for number, input in enumerate(inputs_100.inputs):
        my_checker = GringottsChecker(input)
        res = my_checker.check_controller()
        # print(res)
        print(f"Output on input number {number + 1}: {res}")
        print('---------------------------------------------------------------------')
        if res[-2] == 'd':
            excedded += 1
        elif res[-1] == 's':
            ...
            passed += 1
   print(f'passed = {passed} , excedeed = {excedded} illegal = {illegal}')

   #
   # res='A'
   # for clause in clouses.clauses[1:]:
   #      res&=clause
   #      cnf_res=to_cnf(res)
   # s = expr("(A | B) & (~B | C) & (~C | ~A)")
   # s = to_cnf(s)
   ...
   # Expected Output: A satisfiable model, e.g., {A: True, B: False, C: True}